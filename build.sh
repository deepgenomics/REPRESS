...
# -----------------------------------------------------------------------------
# Script to download and prepare genomic reference data for use with genome_kit.
#
# It performs the following:
# - Creates a temporary working directory
# - Downloads genome FASTA and annotation files from UCSC, GENCODE, NCBI, and Ensembl
# - Generates genome hash files for each reference using genome_kit
# - Builds genome annotation databases for human (hg19, hg38.p12), mouse (mm39), and rat (rn6)
# - Writes all downloaded and processed data to the user's genome_kit data directory
#
# Requirements:
# - Python environment with genome_kit and appdirs installed (see environment.yml)
# - Internet access to download reference genomes and annotations
#
# Note: This script is idempotent if run multiple times, but does not check for existing files (will redownload and overwrite).
# -----------------------------------------------------------------------------
...
wget https://hgdownload.soe.ucsc.edu/goldenPath/hg19/bigZips/hg19.chromAlias.txt
write_hash_file "hg19"
